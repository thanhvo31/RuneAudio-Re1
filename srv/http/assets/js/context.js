...
$( '.contextmenu a' ).click( function( e ) {
	...
	if ( cmd === 'radiosave' ) { // unsaved webradio (dirble)
		...
		$.post( 'commands.php', { bash: [
			  'redis-cli set updatestart '+ Date.now()
		...
		$.post( 'commands.php', { bash: 'sed -i "'+ plline +' d" "/srv/http/assets/img/playlists/'+ plname +'"' } );
		GUI.list.li.remove();
	} else if ( cmd === 'similar' ) {
...
function removeRadioCoverart() {
	...
	info( {
		...
		, ok          : function() {
			$.post( 'commands.php', { bash: 'echo "'+ name +'" > "/srv/http/assets/img/webradios/'+ urlname +'"' } );
...
function webRadioSave( name, url ) {
	...
	$.post( 'commands.php', { bash: "test -e '/srv/http/assets/img/webradios/"+ urlname +"' && echo 1" }, function( data ) {
		if ( data ) {
...
function addPlaylist( name, oldname ) {
	if ( oldname ) {
		...
		$.post( 'commands.php', { saveplaylist: name.replace( /"/g, '\\"' ) } );
...
function playlistDelete() {
	info( {
		...
		, ok      : function() {
			...
			GUI.list.li.remove();
			$.post( 'commands.php', { bash: 'rm "/srv/http/assets/img/playlists/'+ GUI.list.name.replace( /"/g, '\\"' ) +'"' }, function() {
		...
		info( {
			...
			, ok        : function() {
				...
				if ( !cue ) {
					...
					kid3 += pathfile;
					var cmd = [ kid3, 'mpc update "'+ path +'"', '/srv/http/count.sh 1' ];
				} else {
					var                         sed  = "sed -i"
													  +" -e '/^PERFORMER/ d'"
													  +" -e '/^REM COMPOSER/ d'"
					...
					var cmd = [ sed, 'mpc update "'+ GUI.list.path.substr( 0, file.lastIndexOf( '/' ) ) +'"', '/srv/http/count.sh 1' ];
				}
