var GUI = {
	...
	, apikeydirble : '134aabbce2878ce0dbfdb23fb3b46265eded085b"'
...
$.post( 'commands.php', { getdisplay: 1, data: 1 }, function( data ) {
	...
	setSwipe();
	$.post( 'status.php', function( status ) {
		GUI.status = status;
		renderPlayback();
		displayPlayback();
		setButton();
...
$( '#displaylibrary' ).click( function() {
	...
	info( {
		...
		, ok       : function () {
			...
			$.post( 'commands.php', { setdisplay: data }, function() {
...
$( '#plconsume' ).click( function() {
	if ( GUI.status.consume ) {
		...
		$( this ).addClass( 'bl' );
		notify( 'Consume Mode', 'On - Remove each song after played.<br><gr>Turn off repeat, random and single mode.</gr>', 'list-ul' );
...
$( '#plclear' ).click( function() {
	...
	info( {
		...
		, ok         : function() {
			...
			renderPlaylist();
			$( '.licover-save' ).remove();
			$.post( 'commands.php', { mpc: [ 'mpc clear', '/usr/bin/rm -f "/srv/http/assets/img/webradiopl/*' ] } );
		}
	} );
} );
...
if ( reboot ) {
	info( {
		...
		, message : 'Reboot to complete:'
				   +'<br><w>'+ reboot +'</w>'
		, oklabel : '<i class="fa fa-reboot"></i>Reboot'
		, okcolor : '#de810e'
		, ok      : function() {
			$.post( 'commands.php', { bash: 'reboot' } );
		}
	} );
}
	...
	if ( document.hidden ) {
		...
		$.post( 'commands.php', { getdisplay: 1, data: 1 }, function( data ) {
			...
			if ( GUI.display.color === color ) {
				$.post( 'commands.php', { getbookmarks: 1 }, function( bookmarks ) {
