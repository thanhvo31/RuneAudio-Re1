...
function getData( options ) {
	...
	if ( GUI.plugin === 'Spotify' ) {
		...
		$.post( 'commands.php', { jamendo: args || '' }, function( data ) {
			dataParse( data, path, querytype );
		}, 'json' );
	}
}
...
function getPlaybackStatus() {
	...
	$.post( 'status.php', { artist: $( '#artist' ).text(), album: $( '#album' ).text() }, function( status ) {
		...
		if ( GUI.playback ) {
			renderPlayback();
			setButton();
		} else if ( GUI.library && !GUI.display.playbackswitch ) {
			setButton();
	...
	GUI.debounce = setTimeout( function() {
		...
		$.post( 'status.php', { statusonly: 1 }, function( status ) {
			...
			if ( status.updating_db ) {
				...
				clearInterval( GUI.intUpdate );
				GUI.intUpdate = false;
				var second = Date.now() - status.updatestart;
				notify( 'Library Update', 'Finished in '+ second2HMS( second / 1000 ), 'library' );
			}
		}, 'json' );
	}, GUI.debouncems );
}
...
function menuPackage( e, $this ) {
	...
	if ( $( e.target ).hasClass( 'submenu' ) ) {
		info( {
			...
			, buttoncolor : '#de810e'
			, button      : function() {
				$.post( 'commands.php', { bash: 'systemctl stop '+ id } );
				$this
					.data( 'active', 0 )
		...
		$.post( 'commands.php', { bash: 'systemctl start '+ id }, function() {
			location.port = 9091;
		} );
		$this
			.data( 'enabled', 1 )
...
function renderLibrary() {
	...
	if ( GUI.display.count ) {
		$( '#db-currentpath span' ).html( '<span class="title">LIBRARY<gr>·</gr></span><span id="li-count"><wh>'+ numFormat( $( '#home-blocks' ).data( 'count' ) ) +'</wh> <i class="fa fa-music"></i></span>' );
	} else {
...
function setButtonToggle() {
	...
	if ( $( '#play-group' ).is( ':visible' ) ) {
		...
		if ( timehide ) {
			...
			$( '#posconsume, #poslibrandom' ).addClass( 'hide' )
		...
		if ( timehide ) {
			...
			$( '#iconsume' ).toggleClass( 'hide', GUI.status.consume === 0 );
			$( '#ilibrandom' ).toggleClass( 'hide', GUI.status.librandom === 0 );
			$( '#posrandom, #posconsume, #poslibrandom' ).addClass( 'hide' )
			$( '#posrepeat' ).attr( 'class', 'fa hide' );
