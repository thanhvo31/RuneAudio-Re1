...
s|\(hsl(\).*\()/\*cgl\*/\)|\1'.$hsg.'60%\2|g
	...
	$enabled = exec( "mpc outputs | grep enabled | awk -F'[()]' '{print $2}'" );
...
} else if ( isset( $_POST[ 'getmount' ] ) ) {
	$result = shell_exec( $sudo.'/df -h | grep /mnt/MPD/ | awk \'{print $6" <a class=\"green\"> &bull; </a><gr>"$1"</gr>&ensp;"$2"B&ensp;"$4"B free"}\'' );
	$devices = shell_exec( "$sudo/fdisk -l -o Device | grep ^/dev/sd" );
	$devices.= shell_exec( "$sudo/grep '/mnt/MPD/NAS/' /etc/fstab | awk '{print $2}' | sed 's/\\040/ /'" ); // convert escaped space character "\040" in fstab
	if ( !$devices ) {
		echo rtrim( $result );
		exit();
	}
	...
	foreach( $devices as $device ) {
		if ( strpos( $result, $device ) === false ) {
			if ( substr( $device, 0, 4 ) === '/dev' ) {
				$result.= $device.'^^/mnt/MPD/USB/'.exec( "$sudo/e2label $device" )."\n";
			} else {
				$result.= exec( "$sudo/grep '".$device."' /etc/fstab | awk '{print $1}' | sed 's/\\040/ /'" ).'^^'.$device."\n";
			}
		}
	}
	echo rtrim( $result );
	...
	if ( isset( $_POST[ 'pwdnew' ] ) ) {
		$hash = password_hash( $_POST[ 'pwdnew' ], PASSWORD_BCRYPT, array( 'cost' => 12 ) );
		echo $redis->set( 'password', $hash );
	} else {
		echo 1;
		session_start();
		$_SESSION[ 'login' ] = 1;
